...
| 技术/模型 | 版本/提供商 | 用途 | 核心集成与特性 |
| :--- | :--- | :--- | :--- |
| **DeepSeek v4 Pro** | DeepSeek / 代理网关 | 核心文本大语言模型 | 驱动智能体对话、RAG 关联上下文推理、简历优化与新媒体文案生成，支持流式 (Streaming) 响应输出 |
| **StepAudio 2.5 ASR** | 阶跃星辰 | 语音识别转文本 (ASR) | 支持流式分块音频上传，极速秒级识别用户语音指令 |
| **StepAudio 2.5 TTS** | 阶跃星辰 | 表现力语音合成 (TTS) | 支持 tone 语气指令（如“兴奋、悲伤”）及表现力语气词，赋予分身拟人温度 |
| **StepAudio 2.5 Chat** | 阶跃星辰 | 语音多模态大模型 | 流式多模态文本-语音混合式响应交互，实现实时人机智能对答 |
| **RAG 检索增强引擎** | 自研单例 | 知识库智能检索 | 基于 TF-IDF 词频及相似度计算，提取上下文融入 System Prompt 驱动拟人化应答 |
| **VAD 语音活动检测** | 自研 Web Audio API | 免唤醒全双工通话判定 | 基于 AnalyserNode 音频流 RMS 能量实时监测，自动识别停顿断句并闭环触发发送 |
...
│   │
│   ├── 📁 services/              # ============== 服务对接层 ==============
│   │   ├── 📄 aiService.ts       # 智能体大模型交互服务
│   │   ├── 📄 avatarCloneService.ts # 数字形象克隆与重绘服务
│   │   ├── 📄 sttService.ts      # 语音转文字（ASR）与 Voice Activity Detection (VAD) 引擎
│   │   ├── 📄 ttsService.ts      # 文字转语音（TTS）与音频播放队列管理服务
│   │   └── 📁 stepfun/           # 阶跃星辰（StepFun）大模型原生接口封装
│   │       ├── 📄 asr.ts         # StepAudio 语音转文字（ASR）接口
│   │       ├── 📄 chat.ts        # StepAudio 大模型混合对话接口
│   │       ├── 📄 tts.ts         # StepAudio 表现力语音合成（TTS）接口
│   │       ├── 📄 realtime.ts    # StepAudio WebSocket 全双工实时音频通话
│   │       └── 📄 types.ts       # StepAudio 接口类型定义
│   │
│   ├── 📁 lib/                   # ============== 工具函数库 ==============
...
### 🎨 模块六：简历工坊（ResumeTemplates & ResumeBuilder）
...
### 📝 项目总结
**Opclaw** 经过持续演进与模块深度整合，已经成长为一个高完整度、美观流畅且具备 AI 深度赋能的 **超级个体数字宇宙（One Person Company Suite）**。主要达成里程碑如下：
1. **多端自适应 Bento 架构**：PC 端提供视差高保真品牌 IP 门面，移动端提供极简 Bento 网格卡片仪表盘。
2. **5 套艺术主题系统**：通过 CSS 自定义属性配合 React Context 实现极简、赛博、艺术、童趣、复古风格的一键零延迟无缝切换。
3. **闭环式 AI 数字分身创建流程**：声音克隆（录制/试听） $\rightarrow$ 形象复刻（风格化重绘） $\rightarrow$ 3D 虚拟数字人交互。
4. **实时免唤醒全双工语音通话**：在 ASR/TTS 基础上引入 Web Audio API 音频流 RMS 检测，实现免按键的 **智能端到端语音活动检测（VAD）** 与流畅对话闭环。
5. **RAG 个人知识库与运营套件**：自研 TF-IDF 相似度检索结合 Tiptap 富文本编辑器、文档导入和 AI 阅读助手，同时附带电商、自媒体、资产管理及 7+ 功能的生活记录空间。
6. **名片硬件互联**：支持 html2canvas 高清名片图片生成与浏览器原生 Web NFC 物理硬件接触式名片互传。

### 🚀 未来展望
1. **数字分身表现力优化**：引入更丰富的 3D 骨骼动画库，优化口型同步（Lip-Sync）与眼神对视等微表情。
2. **大模型能力扩展**：接入更多多模态 API（如多模态视觉识别与文档解析），增强数字分身的场景理解能力。
3. **硬件级交互集成**：进一步拓展 Web NFC / Bluetooth 等物联网 API，在实体周边与线下场景建立物理名片生态。
4. **离线优先设计**：通过 Service Workers 与 IndexedDB 缓存核心静态资产与轻量级 RAG 数据库，确保在网络不稳定时的基本可访问性。
