...
import { 
  PenTool, ShoppingCart, ArrowLeft, Plus,
...
function NewMediaModule() {
  ...
  // 返回工作助手主页
  const handleBack = () => {
    navigate('/assets?tab=work', { replace: true })
  }
...
interface WorkAssistantProps {
  defaultModule?: 'media' | 'ecommerce'
}

export default function WorkAssistant({ defaultModule }: WorkAssistantProps) {
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()
  const [selectedModule, setSelectedModule] = useState<'media' | 'ecommerce' | undefined>(defaultModule)

  // 如果直接指定了模块，显示对应模块；否则显示选择界面
  const currentModule = defaultModule || selectedModule

  // 处理模块选择
  const handleSelectModule = (module: 'media' | 'ecommerce') => {
    setSelectedModule(module)
    // 更新 URL 参数以保持一致性
    const tab = searchParams.get('tab')
    if (tab === 'work') {
      navigate(`/assets?tab=work-${module}`, { replace: true })
    }
  }
      ...
      <div className="min-h-screen pb-8">
        ...
        {currentModule ? (
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            ...
            >
              <h1 className="text-3xl font-bold mb-2">工作助手</h1>
              <p className="text-text-muted">选择您要使用的功能模块</p>
            </motion.div>
            ...
            <div className="grid gap-6 max-w-4xl mx-auto md:grid-cols-2">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => handleSelectModule('media')}
                className="glass-card p-8 text-left group relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-violet-500/20 to-purple-600/20 rounded-full blur-3xl -mr-16 -mt-16" />
                ...
                <div className="relative">
                  ...
                  <h2 className="text-2xl font-bold mb-2">新媒体</h2>
                  <p className="text-text-muted mb-4">社交媒体内容创作和管理</p>
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02, y: -4 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => handleSelectModule('ecommerce')}
                className="glass-card p-8 text-left group relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-orange-500/20 to-amber-600/20 rounded-full blur-3xl -mr-16 -mt-16" />
                
                <div className="relative">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center mb-4 shadow-lg">
                    <ShoppingCart size={32} className="text-white" />
                  </div>
                  
                  <h2 className="text-2xl font-bold mb-2">电商运营</h2>
                  <p className="text-text-muted mb-4">电商平台管理和数据分析</p>
